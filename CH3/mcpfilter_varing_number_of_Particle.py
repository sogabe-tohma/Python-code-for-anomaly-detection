...
class ParticleFilter(object):
    ...
    def draw_graph1(self):
        # グラフ描画
        T = len(self.y)
        #pred = self.get_filtered_value().reshape(50,1)
        plt.figure(figsize=(5,4))
        plt.plot(df, "--")
        #plt.plot(pred, "k")


        #図3.26の(a)と(b)を作成するために、
        for t in range(T):
            plt.scatter(np.ones(self.n_particle)*t, self.x[t],facecolors='none', color="k", s=8)
        #plt.show()
